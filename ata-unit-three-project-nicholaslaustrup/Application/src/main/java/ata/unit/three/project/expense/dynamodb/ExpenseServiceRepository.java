...
public class ExpenseServiceRepository {
    ...
    public void removeExpenseItemToList(String id, ExpenseItem item) {
        ExpenseItemList list = mapper.load(ExpenseItemList.class, id);
        if (list.getExpenseItems() != null) { // Eduardo's code below
            for (int i = 0; i < list.getExpenseItems().size(); i++){
                if (list.getExpenseItems().get(i).getId().equals(item.getId())){
                    list.getExpenseItems().remove(i);
                    break;
                }
            }
            mapper.save(list);
    ...
    public ExpenseItemList getExpenseListById(String id) {
        System.out.println("expense service repository get expense");
