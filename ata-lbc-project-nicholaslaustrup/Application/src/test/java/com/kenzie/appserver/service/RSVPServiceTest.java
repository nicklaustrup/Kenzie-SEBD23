...
public class RSVPServiceTest {
    ...
    public void findAllRSVPs() {
        ...
        @Test
    public void updateRsvp_successful(){
                // GIVEN
            RSVPCompositeId id = new RSVPCompositeId();
            id.setUserId("1");
            id.setEventId("2");
            ...
            RSVPRecord rsvpRecord = new RSVPRecord();
            rsvpRecord.setUserId("1");
            rsvpRecord.setEventId("2");
            rsvpRecord.setIsAttending(true);

            RSVPRecord rsvpRecord2 = new RSVPRecord();
            rsvpRecord2.setUserId("1");
            rsvpRecord2.setEventId("2");
            rsvpRecord2.setIsAttending(false);

            when(rsvpRepository.save(rsvpRecord)).thenReturn(rsvpRecord);
            RSVP rsvp = rsvpService.addNewRSVP(new RSVP("1", "2", true));
            assertEquals((rsvpRecord.getUserId()), rsvp.getUserId(), "RSVP User IDs do not match");
            assertEquals((rsvpRecord.getEventId()), rsvp.getEventId(), "RSVP Event IDs do not match");
            assertEquals((rsvpRecord.getIsAttending()), rsvp.getIsAttending(), "RSVP isAttendings do not match");


            when(rsvpRepository.save(rsvpRecord2)).thenReturn(rsvpRecord2);
            RSVP rsvp2 = rsvpService.updateRSVP(new RSVP("1", "2", false));
            assertEquals((rsvpRecord2.getUserId()), rsvp2.getUserId(), "Updated RSVP User IDs do not match");
            assertEquals((rsvpRecord2.getEventId()), rsvp2.getEventId(), "Updated RSVP Event IDs do not match");
            assertEquals((rsvpRecord2.getIsAttending()), rsvp2.getIsAttending(), "Updated RSVP isAttendings do not match");
