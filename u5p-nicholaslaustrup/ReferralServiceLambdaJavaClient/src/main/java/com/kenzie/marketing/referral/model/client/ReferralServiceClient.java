...
public class ReferralServiceClient {
    ...
    private static final String DELETE_REFERRALS_ENDPOINT = "referral/delete";
    ...
    public boolean deleteReferrals(List<String> customerIds){
        ...
        try {
            request = mapper.writeValueAsString(customerIds);
        } catch(JsonProcessingException e) {
            throw new ApiGatewayException("Unable to serialize request: " + e);
        }
        
        String response = endpointUtility.postEndpoint(DELETE_REFERRALS_ENDPOINT, request);
