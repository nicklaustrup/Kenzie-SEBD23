...
//                                "employee.csv");
////                    }
                    employeeData = downloadEmployeeData(new URL("https://java2-kenzieacademy-donotdelete.s3.amazonaws.com/employee.csv"), "employee.csv");
                } catch (Exception ex) {
                ...
                while (((values = reader.readMap())) != null) {
                    ...
                    if (theCorrectUser(request.getEmployeeUserName(), userName)) {
                        if (inTheSameDepartment(user.getDepartment().getName(), department)) {
                            ...
                            //Audit log
                            log.info("Audit: User " + request.getRequesterUserName() + " viewed employee " +
                                    request.getEmployeeUserName() + " paycheck information.");
                        } else {
